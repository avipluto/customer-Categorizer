...
class ModelEvaluation:
    ...
    def evaluate_model(self) -> EvaluateModelResponse:
        try:
            ...
            # NOTE: get_best_model() itself can throw if AWS credentials aren't
            # configured yet (e.g. AWS_ACCESS_KEY_ID not set), since constructing
            # the S3 client happens immediately. Until real AWS/S3 setup is done,
            # treat any failure here as "no previous best model to compare against"
            # rather than crashing the whole training pipeline.
            try:
                best_model = self.get_best_model()
            except Exception as e:
                logging.info(f"Could not check for existing best model (AWS/S3 likely not configured yet): {e}")
                best_model = None

            if best_model is not None:
                # NOTE: wrapped in try/except because the S3 bucket referenced by
                # TRAINING_BUCKET_NAME may contain a stale/incompatible model from
                # earlier experimentation (e.g. trained on a different column set).
                # If comparing against it fails, we simply skip the comparison and
                # accept the newly trained model on its own merits instead of
                # crashing the whole training pipeline.
                try:
                    ...
                    logging.info(f"Could not evaluate existing best model (likely stale/incompatible): {e}")
                    best_model_f1_score = None
                    best_model_metric_artifact = None
